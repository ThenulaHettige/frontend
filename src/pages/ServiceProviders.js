          ...
          >
            <div className="p-4">
              ...
              <div className="mt-4 space-y-2">
                ...
                <p className="text-sm text-gray-600">
                  <span className="font-medium">District:</span> {provider.district}
                </p>
                <p className="text-sm text-gray-600">
                  <span className="font-medium">Hourly Rate:</span> {provider.hourlyRate}
                </p>
              </div>
                ...
                <div className="flex items-center">
                  {[...Array(5)].map((_, index) => (
                    <Star
                      key={index}
                      className={`h-4 w-4 ${
                        index < Math.floor(provider.reviews.reduce((acc, review) => acc + review.rating, 0) / provider.reviews.length)
              ...
              <button
                ...
                onClick={() => window.location.href = `/service-providers/${provider.id}`}
              >
                View Profile
              </button>
            </div>
